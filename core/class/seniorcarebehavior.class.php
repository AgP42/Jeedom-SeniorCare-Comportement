...
class seniorcarebehavior extends eqLogic {
    ...
    public static function buttonAlert($_option) { // fct appelée par le listener des buttons d'alerte, n'importe quel bouton arrive ici
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un trigger d\'un bouton d\'alerte ############');

      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcarebehavior->execActions('action_alert_bt'); // on appelle les actions definies pour cette personne pour les boutons d'alertes
    ...
    public static function buttonAlertCancel($_option) { // fct appelée par le listener des buttons d'annulation d'alerte, n'importe quel bouton arrive ici
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un trigger d\'un bouton d\'annulation d\'alerte ############');

      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcarebehavior->execActions('action_cancel_alert_bt'); // on appelle les actions definies pour cette personne pour les boutons d'alertes
    ...
    public static function sensorSecurity($_option) { // fct appelée par le listener des capteurs de sécurité, n'importe quel capteur arrive ici
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un trigger de sécurité ############');

      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']); // on cherche la personne correspondant au bouton d'alerte
      foreach ($seniorcarebehavior->getConfiguration('security') as $security) { // on boucle direct dans la conf
        if ('#' . $_option['event_id'] . '#' == $security['cmd']) { // on cherche quel est l'event qui nous a déclenché pour pouvoir chopper son nom et type (utilisé pour les tags)
          ...
          log::add('seniorcarebehavior', 'debug', 'boucle capteurs security, name : ' . $security['name'] . ' - cmd : ' . $security['cmd']  . ' - ' . $security['sensor_security_type']);
          $seniorcarebehavior->execActions('action_security', $security['name'], $security['sensor_security_type']); // on appelle les actions definies pour cette personne
    ...
    public static function sensorSecurityCancel($_option) { // fct appelée par le listener des boutons d'annulation de l'alerte de sécurité, n'importe quel capteur arrive ici
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un bouton d\'annulation d\'alerte de sécurité ############');

      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']); // on cherche la personne correspondant au bouton d'alerte
      $seniorcarebehavior->execActions('action_cancel_security'); // on appelle les actions definies pour cette personne

    }

    public static function checkAndActionSeuilsSensorConfort($seniorcarebehavior, $_name, $_cmd, $_seuilBas, $_seuilHaut, $_type) { // appelée soit par le cron15, soit par un listener (via la fct sensorConfort - desactivée), va regarder si on est dans les seuils définis et si non appliquer les actions voulues
    ...
    // TODO on pourrait ajouter la date de collecte de la valeur pour ne pas faire des alertes sur une vieille info, ou au contraire ajouter une alerte si pas de valeur fraiche pendant un certain temps. Mais ça peut etre aussi géré par le core dans les configuration de la cmd...
      ...
      $now = time();
      $rep_warning = $seniorcarebehavior->getConfiguration('repetition_warning');
      $tempsDepuisActionWarningConfort = $now - $seniorcarebehavior->getCache('actionWarningConfortStartTimestamp' . $_cmd); // on garde 1 cache par cmd
      $warningConfortLauched = $seniorcarebehavior->getCache('WarningConfortLauched' . $_cmd);

      $valeur = jeedom::evaluateExpression($_cmd);

      log::add('seniorcarebehavior', 'debug', 'Fct checkAndActionSeuilsSensorConfort, name : ' . $_name . ' - ' . $_type . ' - ' . $_cmd . ' - ' . $valeur . ' - ' . $_seuilBas . ' - ' . $_seuilHaut . ' - Rep warning : ' . $seniorcarebehavior->getConfiguration('repetition_warning'));

      log::add('seniorcarebehavior', 'debug', 'Fct checkAndActionSeuilsSensorConfort, WarningConfortLauched : ' . $warningConfortLauched . ' - last action lancé il y a (min) : ' . $tempsDepuisActionWarningConfort / 60);

      if(!is_numeric($valeur)){

        log::add('seniorcarebehavior', 'debug', 'Capteurs confort :' . $_name . ' la valeur est pas numerique, on fait rien !');
        ...
        log::add('seniorcarebehavior', 'debug', 'Capteurs confort :' . $_name . ' dans les seuils, on fait rien');
        ...
        log::add('seniorcarebehavior', 'debug', 'Capteurs confort :' . $_name . ' retour à la normal !');
        $seniorcarebehavior->setCache('WarningConfortLauched' . $_cmd, false); // on remet dans le cache qu'on a pas lancé les actions
        $seniorcarebehavior->execActions('action_cancel_warning_confort', $_name, $_type, $valeur, $_seuilBas, $_seuilHaut); // appel de la boucle d'execution des actions avec les infos pour les tag des messages
        return 1;
        ...
        log::add('seniorcarebehavior', 'debug', 'Capteurs confort :' . $_name . ' est hors seuils, tempsDepuisActionWarningConfort : ' . $tempsDepuisActionWarningConfort . ' warningConfortLauched : ' . $warningConfortLauched . ' rep_warning : ' . $rep_warning);

        $seniorcarebehavior->setCache('WarningConfortLauched' . $_cmd, true); // on garde en cache qu'on a lancé nos actions au moins 1 fois pour cette commande
        $seniorcarebehavior->setCache('actionWarningConfortStartTimestamp' . $_cmd, $now); // on memorise l'heure du lancement du warning

        $seniorcarebehavior->execActions('action_warning_confort', $_name, $_type, $valeur, $_seuilBas, $_seuilHaut); // on execute toutes les actions
        ...
        log::add('seniorcarebehavior', 'debug', 'Capteurs confort :' . $_name . ' est hors seuils, mais déjà lancé les actions de warning');
...
/*    public static function sensorConfort($_option) { // fct appelée par le listener des capteurs conforts (on sait pas lequel, ça serait trop simple, mais on connait l'event_id et la valeur).
      ...
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un changement d\'un capteur confort ############');

    //  log::add('seniorcarebehavior', 'debug', 'Fct sensorConfort appelé par le listener : $_option[seniorcarebehavior_id] : ' . $_option['seniorcarebehavior_id'] . ' - value : ' . $_option['value'] . ' - event_id : ' . $_option['event_id']);

      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']);
      if (is_object($seniorcarebehavior) && $seniorcarebehavior->getIsEnable() == 1 ) {
        foreach ($seniorcarebehavior->getConfiguration('confort') as $confort) { // on boucle direct dans la conf
          if ('#' . $_option['event_id'] . '#' == $confort['cmd']) { // on cherche quel est l'event qui nous a déclenché

          //  log::add('seniorcarebehavior', 'debug', 'Fct sensorConfort appelé par le listener, name : ' . $confort['name'] . ' - cmd : ' . $confort['cmd']  . ' - ' . $confort['sensor_confort_type'] . ' - ' . $confort['seuilBas'] . ' - ' . $confort['seuilHaut']);

            if($confort['seuilBas'] != '' || $confort['seuilHaut'] != '') { // si les seuils sont definis (on set le listener de toutes facons maintenant)
              $seniorcarebehavior->checkAndActionSeuilsSensorConfort($seniorcarebehavior, $confort['name'], $_option['value'], $confort['seuilBas'], $confort['seuilHaut'], $confort['sensor_confort_type']);
            }
    ...
    public static function sensorLifeSign($_option) { // fct appelée par le listener des capteurs d'activité, n'importe quel capteur arrive ici
      log::add('seniorcarebehavior', 'debug', '################ Detection d\'un capteur d\'activité ############');


      $seniorcarebehavior = seniorcarebehavior::byId($_option['seniorcarebehavior_id']);
      $seniorcarebehavior->setCache('lastLifeSignTimestamp', time()); // on met en cache le timestamp à l'heure du dernier event. C'est le cron qui regardera toutes les min si on est hors timer

      // on recupere l'état des des warning et alertes
      $actionWarningLifeSignLaunched = $seniorcarebehavior->getCache('actionWarningLifeSignLaunched');
      $actionAlertLifeSignLaunched = $seniorcarebehavior->getCache('actionAlertLifeSignLaunched');

      log::add('seniorcarebehavior', 'debug', 'Fct sensorLifeSign appelé par le listener, seniorcarebehavior_id : ' . $_option['seniorcarebehavior_id'] . ' - value : ' . $_option['value'] . ' - event_id : ' . $_option['event_id'] . ' - timestamp mis en cache : ' . time() . ' cache lu : ' . $actionWarningLifeSignLaunched . ' - '. $actionAlertLifeSignLaunched);

      if ($actionWarningLifeSignLaunched){ // si on était en phase d'avertissement, on lance les actions d'arret warning
        $seniorcarebehavior->execActions('action_desactivate_warning_life_sign');
      }

      if ($actionAlertLifeSignLaunched){ // si on était en phase d'alerte, on lance les actions d'arret alerte
        $seniorcarebehavior->execActions('action_desactivate_alert_life_sign');
      }

      // dans tous les cas on declare qu'on est pas en phase de warning ni d'alerte, puisqu'on vient de recevoir un signe de vie
      $seniorcarebehavior->setCache('actionWarningLifeSignLaunched', false);
      $seniorcarebehavior->setCache('actionAlertLifeSignLaunched', false);
    ...
    public static function cron() { //executée toutes les min par Jeedom
      ...
      log::add('seniorcarebehavior', 'debug', '#################### CRON ###################');

      //pour chaque equipement (personne) declaré par l'utilisateur
      foreach (self::byType('seniorcarebehavior',true) as $seniorcarebehavior) {

        if (is_object($seniorcarebehavior) && $seniorcarebehavior->getIsEnable() == 1) { // si notre eq existe et est actif
          //TODO : c'est ici qu'il faudra gerer l'absence de la personne de son logement

          /********* Gestion de l'onglet "détection d'inactivité" ********/

          $lifeSignDetectionDelay = $seniorcarebehavior->getConfiguration('life_sign_timer') * 60; // on va lire la durée des timers dans la conf et on le met en secondes
          $lifeSignWarningDelay = $seniorcarebehavior->getConfiguration('warning_life_sign_timer') * 60;

          $lastLifeSignTimestamp = $seniorcarebehavior->getCache('lastLifeSignTimestamp'); // on va lire le timestamp du dernier trigger, en secondes
          $actionWarningLifeSignTimestamp = $seniorcarebehavior->getCache('actionWarningLifeSignTimestamp'); // on va lire le timestamp du lancement du warning, en secondes
          ...
          $actionWarningLifeSignLaunched = $seniorcarebehavior->getCache('actionWarningLifeSignLaunched'); // on recupere l'état des des warning et alertes
          $actionAlertLifeSignLaunched = $seniorcarebehavior->getCache('actionAlertLifeSignLaunched');

          log::add('seniorcarebehavior', 'debug', 'Cache lu : ' . $actionWarningLifeSignLaunched . ' - ' . $actionAlertLifeSignLaunched);
          ...
          //= le premier timer est échu mais aucune action ni warning si alerte n'est en cours --> on va lancer les actions warning
            log::add('seniorcarebehavior', 'debug', 'Actions Warning Life Sign A lancer. Timer lu : ' . $lifeSignDetectionDelay . ', sec depuis last event : ' . $secSinceLastLifeSign);

            $seniorcarebehavior->execActions('action_warning_life_sign');

            $seniorcarebehavior->setCache('actionWarningLifeSignLaunched', true); // on memorise qu'on a lancé les actions pour ne pas repeter toutes les min
            $seniorcarebehavior->setCache('actionWarningLifeSignTimestamp', $now); // on memorise l'heure du lancement du warning
            ...
            log::add('seniorcarebehavior', 'debug', 'Actions Alerte Life Sign à lancer, temps depuis last event : ' . $secSinceLastLifeSign . ', sec depuis le lancement du warning : ' . $secSinceWarningLifeSign);

            // lance les actions alertes
            $seniorcarebehavior->execActions('action_alert_life_sign');

            $seniorcarebehavior->setCache('actionAlertLifeSignLaunched', true); // on memorise qu'on a lancé les actions d'alertes
    ...
    // Sert ici pour les capteurs conforts
      public static function cron15() {
        ...
        log::add('seniorcarebehavior', 'debug', '#################### CRON 15 ###################');
        ...
        foreach (self::byType('seniorcarebehavior',true) as $seniorcarebehavior) {
          ...
          if (is_object($seniorcarebehavior) && $seniorcarebehavior->getIsEnable() == 1) { // si notre eq existe et est actif
            ...
            foreach ($seniorcarebehavior->getConfiguration('confort') as $confort) { // on boucle direct dans la conf
              ...
              log::add('seniorcarebehavior', 'debug', 'Cron15 boucle capteurs confort, name : ' . $confort['name'] . ' - cmd : ' . $confort['cmd']  . ' - ' . $confort['sensor_confort_type'] . ' - ' . $confort['seuilBas'] . ' - ' . $confort['seuilHaut']);
              ...
              if($confort['seuilBas'] != '' || $confort['seuilHaut'] != '') { // évalue si on a au moins 1 seuil defini (de toute facon on peut pas n'en remplir qu'1 des deux)
                ...
                $etatSensor *= $seniorcarebehavior->checkAndActionSeuilsSensorConfort($seniorcarebehavior, $confort['name'], $confort['cmd'], $confort['seuilBas'], $confort['seuilHaut'], $confort['sensor_confort_type']);
                log::add('seniorcarebehavior', 'debug', 'Cron15 boucle capteurs confort, etatSensor : ' . $etatSensor);
            ...
            if($etatSensor){ // ils ont tous repondu 1, on va lancer les actions
              $seniorcarebehavior->execActions('action_cancel_all_warning_confort'); // appel de la boucle d'execution des actions avec les infos pour les tag des messages
            }
    ...
    public function execActions($_config, $_sensor_name = NULL, $_sensor_type = NULL, $_sensor_value = NULL, $_seuilBas = NULL, $_seuilHaut = NULL) { // on donne le type d'action en argument et ca nous execute toute la liste. Les autres arguments sont pour les tag des messages si applicable
      ...
      log::add('seniorcarebehavior', 'debug', '################ Execution des actions du type ' . $_config . ' pour ' . $this->getName() .  ' ############');
      ...
      foreach ($this->getConfiguration($_config) as $action) { // on boucle pour executer toutes les actions définies
        try {
          ...
          log::add('seniorcarebehavior', 'error', $this->getHumanName() . __(' : Erreur lors de l\'éxecution de ', __FILE__) . $action['cmd'] . __('. Détails : ', __FILE__) . $e->getMessage());
        }
      } //*/
    ...
    public function cleanAllListener() {
      ...
      log::add('seniorcarebehavior', 'debug', 'Fct cleanAllListener pour : ' . $this->getName());

      $listeners = listener::byClass('seniorcarebehavior'); // on prend tous nos listeners de ce plugin, pour toutes les personnes
      foreach ($listeners as $listener) {
        $seniorcarebehavior_id_listener = $listener->getOption()['seniorcarebehavior_id'];

    //    log::add('seniorcarebehavior', 'debug', 'cleanAllListener id lue : ' . $seniorcarebehavior_id_listener . ' et nous on est l id : ' . $this->getId());

        if($seniorcarebehavior_id_listener == $this->getId()){ // si on correspond a la bonne personne, on le vire
          $listener->remove();
        }
...
/*      $listener = listener::byClassAndFunction('seniorcarebehavior', 'sensorLifeSign', array('seniorcarebehavior_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcarebehavior', 'buttonAlert', array('seniorcarebehavior_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcarebehavior', 'sensorConfort', array('seniorcarebehavior_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      }
      $listener = listener::byClassAndFunction('seniorcarebehavior', 'sensorSecurity', array('seniorcarebehavior_id' => intval($this->getId())));
      if (is_object($listener)) {
        $listener->remove();
      } //*/
    ...
    public function postSave() {
      ...
      foreach ($jsSensors as $key => $jsSensor) { // on boucle dans tous nos types de capteurs pour recuperer les infos
        log::add('seniorcarebehavior', 'debug', 'Boucle de $jsSensors : key : ' . $key);
        ...
        if (is_array($this->getConfiguration($key))) {
          foreach ($this->getConfiguration($key) as $sensor) {
            if ($sensor['name'] != '' && $sensor['cmd'] != '') { // si le nom et la cmd sont remplis
              ...
              log::add('seniorcarebehavior', 'debug', 'Capteurs sensor config lue : ' . $sensor['name'] . ' - ' . $sensor['cmd']);
      ...
      foreach ($jsSensors as $key => $jsSensor) { // on boucle dans tous nos types de capteurs. $key va prendre les valeurs suivantes : life_sign, alert_bt, confort puis security
        ...
        foreach ($jsSensor as $sensor) { // pour chacun des capteurs de ce type
          ...
          log::add('seniorcarebehavior', 'debug', 'New Capteurs config : type : ' . $key . ', sensor name : ' . $sensor['name'] . ', sensor cmd : ' . $sensor['cmd']);
      ...
      if ($this->getIsEnable() == 1) { // si notre eq est actif, on va lui definir nos listeners de capteurs
        ...
        //    $listenerFunction = 'sensorConfort';
          ...
          $listener = listener::byClassAndFunction('seniorcarebehavior', $listenerFunction, array('seniorcarebehavior_id' => intval($this->getId())));
          if (!is_object($listener)) { // s'il existe pas, on le cree, sinon on le reprend
            $listener = new listener();
            $listener->setClass('seniorcarebehavior');
            $listener->setFunction($listenerFunction); // la fct qui sera appellée a chaque evenement sur une des sources écoutée
            $listener->setOption(array('seniorcarebehavior_id' => intval($this->getId())));
          }
          ...
          log::add('seniorcarebehavior', 'debug', 'sensor listener set - cmd :' . $cmd->getHumanName() . ' - event : ' . $cmd->getValue());
...
class seniorcarebehaviorCmd extends cmd {
   ...
   /*   if ($this->getLogicalId() == 'sensor_confort') {
        log::add('seniorcarebehavior', 'debug', 'Fct execute - Capteurs confort, valeur renvoyée : ' . round(jeedom::evaluateExpression($this->getValue()), 1));
      ...
      if ($this->getLogicalId() == 'sensor_alert_bt' || $this->getLogicalId() == 'sensor_life_sign' || $this->getLogicalId() == 'sensor_security') {
        log::add('seniorcarebehavior', 'debug', 'Fct execute - sensor_alert_bt or sensor_life_sign or sensor_security, valeur renvoyée : ' . jeedom::evaluateExpression($this->getValue()));
        return jeedom::evaluateExpression($this->getValue());
      } //*/
